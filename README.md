...
- **Allow unrestricted browser control** is an explicit global opt-in. While enabled, page reads, page actions, and tab list/follow/close operations run without approval prompts; disabling it immediately cancels in-flight browser tool calls and restores the page-sharing and approval rules above. Browser-protected DOM content remains inaccessible in either mode.
