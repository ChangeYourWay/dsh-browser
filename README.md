...
- **Allow unrestricted browser control** is an explicit global opt-in. While enabled, page reads, page actions, and tab list/follow/close operations run without approval prompts. Disabling it cancels calls that have not dispatched an action, waits for already-dispatched browser operations to settle, and only then saves the restrictive setting; later calls use the page-sharing and approval rules above immediately. Browser-protected DOM content remains inaccessible in either mode.
