...
- **Unrestricted control is explicit**: enabling **Allow unrestricted browser control** lets page reads, page actions, and tab list/follow/close operations run without confirmation. Disabling it cancels calls that have not dispatched an action, waits for already-dispatched browser operations to settle, and only then saves the restrictive setting; later calls use proportional approval immediately. Protected-page DOM content remains inaccessible.
